    ...
    <Layout title="Upload Resume">
      <div className="max-w-4xl mx-auto animate-fade-in">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-white tracking-tight">Upload Candidates</h2>
            <p className="text-slate-400 text-sm mt-1">PDF files only, max 5MB each</p>
          </div>

          {/* Mode Toggle */}
          <div className="flex bg-white/5 border border-white/10 rounded-xl p-1.5 self-start sm:self-auto">
            <button
              onClick={() => setMode('single')}
              className={`px-5 py-2 text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-2 ${
                mode === 'single' ? 'gradient-primary text-white shadow-glow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              <User size={16} /> Single
            </button>
            <button
              ...
              className={`px-5 py-2 text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-2 ${
                mode === 'bulk' ? 'gradient-primary text-white shadow-glow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              <Files size={16} /> Bulk
            </button>
        ...
        {/* ─── SINGLE MODE ─── */}
        {mode === 'single' && (
          <div className="glass-strong border border-white/10 rounded-2xl p-8 shadow-glass-lg flex flex-col gap-6 animate-scale-in">

            {/* Drag Drop FIRST so user drops file → form auto-fills */}
            <div>
              <label className="text-sm text-slate-400 mb-1.5 block font-medium">Resume PDF *</label>
              <div
                ...
                <div className="relative z-10 flex flex-col items-center">
                  {parsing ? (
                    <>
                      ...
                      <p className="text-white font-medium">Parsing resume...</p>
                      <p className="text-slate-500 text-sm mt-1">Extracting details using AI</p>
                    </>
                  ) : file ? (
                    <>
                    ...
                    <>
                      ...
                      <p className="text-white font-medium text-lg">Drop PDF to auto-fill form</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <label className="text-sm text-slate-400 mb-1.5 block font-medium">Phone</label>
                <input
                  value={phone}
              ...
              <div>
                <label className="text-sm text-slate-400 mb-1.5 block font-medium">Job Position</label>
                <select
                  value={jobId}
        ...
        {mode === 'bulk' && (
          <div className="glass-strong border border-white/10 rounded-2xl p-8 shadow-glass-lg flex flex-col gap-6 animate-scale-in">
            ...
            {/* Job for all */}
            <div>
              <label className="text-sm text-slate-400 mb-1.5 block font-medium">Job Position (applies to all)</label>
              <select
                value={bulkJobId}
            ...
            >
              ...
              <p className="text-white font-medium text-lg">Drop multiple PDFs here</p>
              <p className="text-slate-400 text-sm mt-1">or click to browse • Up to 20 files</p>
            </div>
            ...
            {bulkFiles.length > 0 && (
              <div className="flex flex-col gap-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {bulkFiles.map((b, i) => (
                  <div key={i} className="bg-slate-900/60 border border-white/5 rounded-xl p-4 transition-all duration-200 hover:border-white/10">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="p-1.5 bg-white/5 rounded-lg border border-white/5 flex-shrink-0">
                          <StatusIcon status={b.status} />
                        </div>
                        <span className="text-slate-300 text-sm font-medium truncate">{b.file.name}</span>
                      </div>
                      <button
                    ...
                    {b.status === 'parsed' || b.status === 'uploading' || b.status === 'done' ? (
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <input
                          ...
                          className="input-glass text-white rounded-lg px-3 py-2 text-xs placeholder-slate-500 disabled:opacity-50"
                        />
                        <input
                          value={b.email || ''}
                      ...
                      <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-lg p-2 mt-2">
                        <AlertCircle size={14} className="text-red-400" />
                        <p className="text-red-400 text-xs font-medium">{b.error}</p>
                      </div>
                    ) : (
